...
module.exports.run = ({ event, api, args, permssion }) => {
    ...
    switch (args[0]) {
        ...
        case"all": {
            ...
            if (msg.length == 0) return api.sendMessage("[ Note ] Your team does not have any note ❌", threadID, messageID);
            api.sendMessage(`Note List of The Group :\n\n${msg}`, threadID, messageID);
            break;
        }
        case "rm":
        case "remove":
        case "delete": {
            if (!isNaN(content) && content > 0) {
                ...
                api.sendMessage(`[ Note ] has successfully deleted - ${content}`, threadID, messageID);
                break;
            }
            else if (content == "all") {
                if (permssion == 0) return api.sendMessage("[Note] You do not have enough powers to be able to use the notes that only administrators are used!", threadID, messageID);
        ...
        default: {
            if (thisThread.listRule.length != 0) {
                var msg = "", index = 0;
                for (const item of thisThread.listRule) msg += `${index+=1}/ ${item}\n`;
                return api.sendMessage(`Note List of The Group :\n\n${msg}`, threadID, messageID);
            }
