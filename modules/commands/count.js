...
module.exports.run = async function ({ api, Threads, Users, event, args, client, __GLOBAL }) {
  ...
  if (input == "") { out(`Please Enter Category - ⚠️\n\nCategory List :\nmessage / admin / male / female / allgroup / alluser\n\nExample : ${global.config.PREFIX}count message - ✅`) }
  if (input == "message") { out(`This Group Has ${threadInfo.messageCount} Messages`) }
  if (input == "admin") { out(`This Bot Have ${threadData.adminIDs.length} Administrators`) }
  if (input == "member") { out(`This Group Has ${participantIDs.length} Members`) }
  ...
  if (input == "allgroup") { out(`Total - ${threadList.length} Group Using This Bot`) }
  if (input == "alluser") { out(`Total - ${userget.length} Users Using This Bot`) }
