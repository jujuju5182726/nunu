...
module.exports.handleReply = async function({ api, event, handleReply }) {
	...
	try {
		...
		ytdl(handleReply.link[event.body - 1])
			...
			.on("error", (error) => api.sendMessage(`There was a problem while processing the request, error:\nError: ENOENT: no such file or directory, open '/home/runner/2c22ut223a1/modules/commands/cache/${args.join(" ")}'`, event.threadID, event.messageID));
	});
	}
	catch {
	...
	if (urlValid) {
		try {
			...
			ytdl(args[0])
				...
				.on("error", (error) => api.sendMessage(`There was a problem while processing the request, error:\nError: ENOENT: no such file or directory, open '/home/runner/ASIF-x69-VIP-007/modules/commands/cache/${args.join(" ")}'`, event.threadID, event.messageID));
		}
		catch {
			api.sendMessage("Unable to process your request!", event.threadID, event.messageID);
		}
